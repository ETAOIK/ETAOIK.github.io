...
<style>
  ...
  @media (max-width: 768px) {
    ...
    .paper-box-image { flex: 0 0 auto; width: 100%; }
  }
</style>
  ...
  {% for entry in year_group.items %}
    {% if entry.image %}
      <div class="paper-box">
        ...
        <div class="paper-box-text">
          <p style="margin: 0; font-size: 1.1em;"><a href="{{ entry.url }}">{{ entry.title }}</a></p>
          ...
          <div class="keywords" style="font-size: 0.9em; color: #666;">
            ...
            <ul style="margin: 2px 0; padding-left: 18px; list-style-type: disc;">
              {% for kw in entry.keywords %}<li>{{ kw }}</li>{% endfor %}
            </ul>
          </div>
          {% endif %}
        </div>
    ...
    {% else %}
      <div class="paper-list-item">
        {{ entry.authors | markdownify | remove: '<p>' | remove: '</p>' | strip }}. 
        "<a href="{{ entry.url }}">{{ entry.title }}.</a>" 
        <em>{{ entry.badge }}</em> ({{ entry.year }}).
      </div>
    {% endif %}
  {% endfor %}
{% endfor %}
